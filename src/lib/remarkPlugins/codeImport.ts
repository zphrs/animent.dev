...
function codeImport(options: CodeImportOptions = {}) {
  ...
  return function transformer(tree: Root, file: VFile) {
    ...
    for (const [node] of codes) {
      ...
      if (!file.dirname) {
        throw new Error(
          '"file" should be an instance of VFile'
        )
      }
      ...
      if (!res || !res.groups || !res.groups.path) {
        throw new Error(
          `Unable to parse file path ${fileMeta}`
        )
      }
      ...
      const fromLine = res.groups.from
        ...
        : undefined
      const hasDash =
        !!res.groups.dash || fromLine === undefined
      ...
      if (!options.allowImportingFromOutside) {
        const relativePathFromRootDir = path.relative(
          rootDir,
          fileAbsPath
        )
        if (
          !rootDir ||
          relativePathFromRootDir.startsWith(
            `..${path.sep}`
          ) ||
